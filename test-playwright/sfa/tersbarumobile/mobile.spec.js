...
async function adbScreenshot(name) {
  try {
    console.log(`📸 Fallback: taking screenshot via adb...`);
    execSync(`"${process.env.ANDROID_HOME || process.env.LOCALAPPDATA + '\\Android\\Sdk'}\\platform-tools\\adb.exe" -s 127.0.0.1:5555 shell screencap -p /sdcard/${name}.png`);
    execSync(`"${process.env.ANDROID_HOME || process.env.LOCALAPPDATA + '\\Android\\Sdk'}\\platform-tools\\adb.exe" -s 127.0.0.1:5555 pull /sdcard/${name}.png ./tersbarumobile/${name}.png`);
