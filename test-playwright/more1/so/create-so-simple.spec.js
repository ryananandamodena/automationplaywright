...
test.describe('MHC - Sales Order Creation (Simple)', () => {
  ...
  test('Create Sales Order - Full Flow', async ({ page }) => {
    ...
    allure.story('Create new sales order with customer and products');
    ...
    allure.parameter('Customer Search Term', 'Dedi');
    ...
    try {
      ...
      // 3. Click Create New
      await allure.step('Open Create Sales Order wizard', async () => {
        console.log('3. Clicking Create New...');
        await page.locator("button:has-text('Create New')").click();
        await page.waitForTimeout(2000);
        const screenshot = await page.screenshot({ path: 'test-results/step1-customer.png', fullPage: true });
        console.log('✓ Create wizard opened');
        
        allure.attachment('Customer Selection Page', screenshot, 'image/png');
      });

    // 4. Select Customer
    await allure.step('Select customer "Dedi"', async () => {
    ...
    await allure.step('Navigate to Products selection', async () => {
      ...
      await page.waitForTimeout(2000);
      const screenshot = await page.screenshot({ path: 'test-results/step2-products.png', fullPage: true });
      console.log('✓ Products page loaded');
      
      allure.attachment('Products Page', screenshot, 'image/png');
    });

    // 6. Add products - find ones with stock
    await allure.step('Add 2 products with available stock', async () => {
      ...
      const screenshot = await page.screenshot({ path: 'test-results/after-add-products.png', fullPage: true });
      
      allure.parameter('Products Added', addedCount);
      allure.attachment('After Adding Products', screenshot, 'image/png');
    });
    ...
    await allure.step('Navigate to Review page', async () => {
      ...
      await page.locator("button:has-text('Next Step')").click();
      await page.waitForTimeout(2000);
      const screenshot = await page.screenshot({ path: 'test-results/step3-review.png', fullPage: true });
      console.log('✓ Review page loaded');
      
      allure.attachment('Review Page', screenshot, 'image/png');
    });

        // 8. Submit
    await allure.step('Submit Sales Order', async () => {
      ...
      const submitBtn = page.locator("button:has-text('Submit')").or(
        ...
        const screenshot = await page.screenshot({ path: 'test-results/order-result.png', fullPage: true });
        
        allure.attachment('Order Submitted', screenshot, 'image/png');
        } else {
