 ...
 * MHC - PPH CRUD TEST
 * 
 * Test CRUD operations untuk menu PPH di Staging Data
 */
...
const LIST_URL = `${BASE_URL}/sync-sap/pph/list`;

test.describe('MHC - PPH CRUD Tests', () => {
  let page;
  ...
  test('1. READ - Should display PPH list', async () => {
    console.log('\n📍 TEST: READ PPH LIST');
    ...
    console.log(`Total PPH records: ${rowCount}`);
    
    await page.screenshot({ path: 'mhc-pph-crud-list.png', fullPage: true });
    console.log('✅ READ: Success');
  });

  test('2. CREATE - Should create new PPH record', async () => {
    console.log('\n📍 TEST: CREATE PPH');
    ...
    if (await addButton.count() > 0) {
      ...
      await page.screenshot({ path: 'mhc-pph-crud-create-form.png', fullPage: true });
  ...
  test('3. SEARCH - Should filter PPH records', async () => {
    console.log('\n📍 TEST: SEARCH PPH');
    ...
    if (await searchInput.count() > 0) {
      ...
      await page.screenshot({ path: 'mhc-pph-crud-search.png', fullPage: true });
  ...
  test('4. UPDATE - Should update PPH record', async () => {
    console.log('\n📍 TEST: UPDATE PPH');
    ...
    if (await editButton.count() > 0) {
      ...
      await page.screenshot({ path: 'mhc-pph-crud-update-form.png', fullPage: true });
  ...
  test('5. DELETE - Should delete PPH record', async () => {
    console.log('\n📍 TEST: DELETE PPH');
  ...
  test('6. VALIDATION - Should show validation errors', async () => {
    ...
    if (await addButton.count() > 0) {
      ...
      if (await submitButton.count() > 0) {
        ...
        await page.screenshot({ path: 'mhc-pph-crud-validation.png', fullPage: true });
