/**
 * MHC - BP ADDRESS CRUD TEST
 * 
 * Test CRUD operations untuk menu BP Address di Staging Data
 */
...
const LIST_URL = `${BASE_URL}/sync-sap/bp-address/list`;

test.describe('MHC - BP Address CRUD Tests', () => {
  let page;
  ...
  test.afterAll(async () => {
    if (createdSnapshot && isAutoCleanupEnabled()) {
      console.log('\n🧹 AUTO CLEANUP BP Address (best effort)');
  ...
  test('1. READ - Should display BP Address list', async () => {
    console.log('\n📍 TEST: READ BP Address LIST');
    ...
    console.log(`Total BP Address records: ${rowCount}`);
    
    await page.screenshot({ path: 'mhc-bp-address-crud-list.png', fullPage: true });
    console.log('✅ READ: Success');
  });

  test('2. CREATE - Should create new BP Address record', async () => {
    console.log('\n📍 TEST: CREATE BP Address');
    ...
    if (await addButton.count() > 0) {
      ...
      await page.screenshot({ path: 'mhc-bp-address-crud-create-form.png', fullPage: true });
  ...
  test('3. SEARCH - Should filter BP Address records', async () => {
    console.log('\n📍 TEST: SEARCH BP Address');
    ...
    if (await searchInput.count() > 0) {
      ...
      await page.screenshot({ path: 'mhc-bp-address-crud-search.png', fullPage: true });
  ...
  test('4. UPDATE - Should update BP Address record', async () => {
    console.log('\n📍 TEST: UPDATE BP Address');
    ...
    if (await editButton.count() > 0) {
      ...
      await page.screenshot({ path: 'mhc-bp-address-crud-update-form.png', fullPage: true });
  ...
  test('5. DELETE - Should delete BP Address record', async () => {
    console.log('\n📍 TEST: DELETE BP Address');
  ...
  test('6. VALIDATION - Should show validation errors', async () => {
    ...
    if (await addButton.count() > 0) {
      ...
      if (await submitButton.count() > 0) {
        ...
        await page.screenshot({ path: 'mhc-bp-address-crud-validation.png', fullPage: true });
