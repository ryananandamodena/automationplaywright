 ...
 * MHC - BP CRUD TEST
 * 
 * Test CRUD operations untuk menu BP di Staging Data
 */
...
const LIST_URL = `${BASE_URL}/sync-sap/bp/list`;

test.describe('MHC - BP CRUD Tests', () => {
  let page;
  ...
  test('1. READ - Should display BP list', async () => {
    console.log('\n📍 TEST: READ BP LIST');
    ...
    console.log(`Total BP records: ${rowCount}`);
    
    await page.screenshot({ path: 'mhc-bp-crud-list.png', fullPage: true });
    console.log('✅ READ: Success');
  });

  test('2. CREATE - Should create new BP record', async () => {
    console.log('\n📍 TEST: CREATE BP');
    ...
    if (await addButton.count() > 0) {
      ...
      await page.screenshot({ path: 'mhc-bp-crud-create-form.png', fullPage: true });
  ...
  test('3. SEARCH - Should filter BP records', async () => {
    console.log('\n📍 TEST: SEARCH BP');
    ...
    if (await searchInput.count() > 0) {
      ...
      await page.screenshot({ path: 'mhc-bp-crud-search.png', fullPage: true });
  ...
  test('4. UPDATE - Should update BP record', async () => {
    console.log('\n📍 TEST: UPDATE BP');
    ...
    if (await editButton.count() > 0) {
      ...
      await page.screenshot({ path: 'mhc-bp-crud-update-form.png', fullPage: true });
  ...
  test('5. DELETE - Should delete BP record', async () => {
    console.log('\n📍 TEST: DELETE BP');
  ...
  test('6. VALIDATION - Should show validation errors', async () => {
    ...
    if (await addButton.count() > 0) {
      ...
      if (await submitButton.count() > 0) {
        ...
        await page.screenshot({ path: 'mhc-bp-crud-validation.png', fullPage: true });
