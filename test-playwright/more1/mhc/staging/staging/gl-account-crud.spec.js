/**
 * MHC - GL ACCOUNT CRUD TEST
 * 
 * Test CRUD operations untuk menu GL Account di Staging Data
 */
...
const LIST_URL = `${BASE_URL}/sync-sap/gl-account/list`;

test.describe('MHC - GL Account CRUD Tests', () => {
  let page;
  ...
  test.afterAll(async () => {
    if (createdSnapshot && isAutoCleanupEnabled()) {
      console.log('\n🧹 AUTO CLEANUP GL Account (best effort)');
  ...
  test('1. READ - Should display GL Account list', async () => {
    console.log('\n📍 TEST: READ GL Account LIST');
    ...
    console.log(`Total GL Account records: ${rowCount}`);
    
    await page.screenshot({ path: 'mhc-gl-account-crud-list.png', fullPage: true });
    console.log('✅ READ: Success');
  });

  test('2. CREATE - Should create new GL Account record', async () => {
    console.log('\n📍 TEST: CREATE GL Account');
    ...
    if (await addButton.count() > 0) {
      ...
      await page.screenshot({ path: 'mhc-gl-account-crud-create-form.png', fullPage: true });
  ...
  test('3. SEARCH - Should filter GL Account records', async () => {
    ...
    if (await searchInput.count() > 0) {
      ...
      await page.screenshot({ path: 'mhc-gl-account-crud-search.png', fullPage: true });
  ...
  test('4. UPDATE - Should update GL Account record', async () => {
    console.log('\n📍 TEST: UPDATE GL Account');
    ...
    if (await editButton.count() > 0) {
      ...
      await page.screenshot({ path: 'mhc-gl-account-crud-update-form.png', fullPage: true });
  ...
  test('5. DELETE - Should delete GL Account record', async () => {
    console.log('\n📍 TEST: DELETE GL Account');
  ...
  test('6. VALIDATION - Should show validation errors', async () => {
    ...
    if (await addButton.count() > 0) {
      ...
      if (await submitButton.count() > 0) {
        ...
        await page.screenshot({ path: 'mhc-gl-account-crud-validation.png', fullPage: true });
