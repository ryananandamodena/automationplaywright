 ...
 */
async function ensureBrowser(browserType: string = DEFAULT_BROWSER): Promise<BrowserSession> {
...
server.tool(
  ...
  async ({ selector, value, label, index, timeout }) => {
    ...
    try {
      ...
      let option: { value?: string; label?: string; index?: number } = {};
      if (value !== undefined) option.value = value;
      if (label !== undefined) option.label = label;
      if (index !== undefined) option.index = index;
 ...
 */
server.tool(
  "playwright_upload_multiple_files",
  {
 ...
 */
server.tool(
  "playwright_upload_file_drag_drop",
  {
 ...
 */
server.tool(
  "playwright_clear_uploaded_files",
  {
    selector: z.string().describe("CSS selector for file input"),
    timeout: z.number().optional().describe("Timeout in milliseconds")
  },
 ...
 */
server.tool(
  "playwright_validate_file_upload",
  {
    ...
    try {
      ...
      if (allowedExtensions && allowedExtensions.length > 0) {
        const isAllowed = allowedExtensions.some(ext => 
          fileExt === ext.toLowerCase() || fileExt === `.${ext.toLowerCase()}`
        );
 ...
 */
server.tool(
  "playwright_get_uploaded_files_info",
  {
    selector: z.string().describe("CSS selector for file input"),
    timeout: z.number().optional().describe("Timeout in milliseconds")
  },
...
server.tool(
  ...
  async ({ selector }) => {
    try {
      ...
      const results: { text: string | null }[] = [];
  ...
  async ({ path, fullPage, selector }) => {
    ...
    try {
      ...
      if (selector) {
        ...
        buffer = await session.page.screenshot({ fullPage: fp, path: p }) as Buffer;
      }
 ...
 */
server.tool(
  "playwright_evaluate_on_element",
  {
    selector: z.string().describe("CSS selector for element"),
  ...
  async ({ selector, script }) => {
    try {
      ...
      const result = await element.evaluate((el: Element, userScript: string) => {
...
server.tool(
  ...
  async ({ key, value }) => {
    try {
      const session = await ensureBrowser();
      await session.page.evaluate(({ k, v }: { k: string; v: string }) => {
 ...
 */
async function executePlaywrightTest(args: string[], cwd: string): Promise<{ success: boolean; output: string; exitCode: number }> {
