 ...
 * MHC - BANK CRUD TEST
 * 
 * Test CRUD operations untuk menu Bank di Staging Data
 */
...
const LIST_URL = `${BASE_URL}/sync-sap/bank/list`;

test.describe('MHC - Bank CRUD Tests', () => {
  let page;
  ...
  test('1. READ - Should display Bank list', async () => {
    console.log('\n📍 TEST: READ Bank LIST');
    ...
    console.log(`Total Bank records: ${rowCount}`);
    
    await page.screenshot({ path: 'mhc-bank-crud-list.png', fullPage: true });
    console.log('✅ READ: Success');
  });

  test('2. CREATE - Should create new Bank record', async () => {
    console.log('\n📍 TEST: CREATE Bank');
    ...
    if (await addButton.count() > 0) {
      ...
      await page.screenshot({ path: 'mhc-bank-crud-create-form.png', fullPage: true });
  ...
  test('3. SEARCH - Should filter Bank records', async () => {
    console.log('\n📍 TEST: SEARCH Bank');
    ...
    if (await searchInput.count() > 0) {
      ...
      await page.screenshot({ path: 'mhc-bank-crud-search.png', fullPage: true });
  ...
  test('4. UPDATE - Should update Bank record', async () => {
    console.log('\n📍 TEST: UPDATE Bank');
    ...
    if (await editButton.count() > 0) {
      ...
      await page.screenshot({ path: 'mhc-bank-crud-update-form.png', fullPage: true });
  ...
  test('5. DELETE - Should delete Bank record', async () => {
    console.log('\n📍 TEST: DELETE Bank');
  ...
  test('6. VALIDATION - Should show validation errors', async () => {
    ...
    if (await addButton.count() > 0) {
      ...
      if (await submitButton.count() > 0) {
        ...
        await page.screenshot({ path: 'mhc-bank-crud-validation.png', fullPage: true });
