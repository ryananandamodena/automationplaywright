 ...
 * MHC - LEGAL CRUD TEST
 * 
 * Test CRUD operations untuk menu Legal di Staging Data
 */
...
const LIST_URL = `${BASE_URL}/sync-sap/legal/list`;

test.describe('MHC - Legal CRUD Tests', () => {
  let page;
  ...
  test.afterAll(async () => {
    if (createdSnapshot && isAutoCleanupEnabled()) {
      console.log('\n🧹 AUTO CLEANUP Legal (best effort)');
  ...
  test('1. READ - Should display Legal list', async () => {
    console.log('\n📍 TEST: READ Legal LIST');
    ...
    console.log(`Total Legal records: ${rowCount}`);
    
    await page.screenshot({ path: 'mhc-legal-crud-list.png', fullPage: true });
    console.log('✅ READ: Success');
  });

  test('2. CREATE - Should create new Legal record', async () => {
    console.log('\n📍 TEST: CREATE Legal');
    ...
    if (await addButton.count() > 0) {
      ...
      await page.screenshot({ path: 'mhc-legal-crud-create-form.png', fullPage: true });
  ...
  test('3. SEARCH - Should filter Legal records', async () => {
    console.log('\n📍 TEST: SEARCH Legal');
    ...
    if (await searchInput.count() > 0) {
      ...
      await page.screenshot({ path: 'mhc-legal-crud-search.png', fullPage: true });
  ...
  test('4. UPDATE - Should update Legal record', async () => {
    console.log('\n📍 TEST: UPDATE Legal');
    ...
    if (await editButton.count() > 0) {
      ...
      await page.screenshot({ path: 'mhc-legal-crud-update-form.png', fullPage: true });
  ...
  test('5. DELETE - Should delete Legal record', async () => {
    console.log('\n📍 TEST: DELETE Legal');
  ...
  test('6. VALIDATION - Should show validation errors', async () => {
    ...
    if (await addButton.count() > 0) {
      ...
      if (await submitButton.count() > 0) {
        ...
        await page.screenshot({ path: 'mhc-legal-crud-validation.png', fullPage: true });
