/**
 * MHC - BP BRANCH CRUD TEST
 * 
 * Test CRUD operations untuk menu BP Branch di Staging Data
 */
...
const LIST_URL = `${BASE_URL}/sync-sap/bp-branch/list`;

test.describe('MHC - BP Branch CRUD Tests', () => {
  let page;
  ...
  test.afterAll(async () => {
    if (createdSnapshot && isAutoCleanupEnabled()) {
      console.log('\n🧹 AUTO CLEANUP BP Branch (best effort)');
  ...
  test('1. READ - Should display BP Branch list', async () => {
    console.log('\n📍 TEST: READ BP Branch LIST');
    ...
    console.log(`Total BP Branch records: ${rowCount}`);
    
    await page.screenshot({ path: 'mhc-bp-branch-crud-list.png', fullPage: true });
    console.log('✅ READ: Success');
  });

  test('2. CREATE - Should create new BP Branch record', async () => {
    console.log('\n📍 TEST: CREATE BP Branch');
    ...
    if (await addButton.count() > 0) {
      ...
      await page.screenshot({ path: 'mhc-bp-branch-crud-create-form.png', fullPage: true });
  ...
  test('3. SEARCH - Should filter BP Branch records', async () => {
    console.log('\n📍 TEST: SEARCH BP Branch');
    ...
    if (await searchInput.count() > 0) {
      ...
      await page.screenshot({ path: 'mhc-bp-branch-crud-search.png', fullPage: true });
  ...
  test('4. UPDATE - Should update BP Branch record', async () => {
    console.log('\n📍 TEST: UPDATE BP Branch');
    ...
    if (await editButton.count() > 0) {
      ...
      await page.screenshot({ path: 'mhc-bp-branch-crud-update-form.png', fullPage: true });
  ...
  test('5. DELETE - Should delete BP Branch record', async () => {
    console.log('\n📍 TEST: DELETE BP Branch');
  ...
  test('6. VALIDATION - Should show validation errors', async () => {
    ...
    if (await addButton.count() > 0) {
      ...
      if (await submitButton.count() > 0) {
        ...
        await page.screenshot({ path: 'mhc-bp-branch-crud-validation.png', fullPage: true });
